...
OProfileJITEventListener::OProfileJITEventListener()
  ...
  if (Agent == NULL) {
    ...
    DOUT(llvm::dbgs() << "Failed to connect to OProfile agent: " << err_str << "\n";
  } else {
    DOUT(llvm::dbgs() << "Connected to OProfile agent.\n";
  }
}
...
OProfileJITEventListener::~OProfileJITEventListener() {
  if (Agent != NULL) {
    if (op_close_agent(Agent) == -1) {
      const std::string err_str = sys::StrError();
      DOUT(llvm::dbgs() << "Failed to disconnect from OProfile agent: " << err_str << "\n";
    } else {
      DOUT(llvm::dbgs() << "Disconnected from OProfile agent.\n";
    }
  }
}
...
static debug_line_info LineStartToOProfileFormat(
  ...
  DOUT(llvm::dbgs() << "Mapping " << reinterpret_cast<void*>(Result.vma) << " to "
