...
static GlobalVariable *SRAGlobal(GlobalVariable *GV, const TargetData &TD,
  ...
  DOUT(llvm::dbgs() << "PERFORMING GLOBAL SRA ON: " << *GV);
...
static bool OptimizeAwayTrappingUsesOfLoads(GlobalVariable *GV, Constant *LV,
  ...
  if (Changed) {
    DOUT(llvm::dbgs() << "OPTIMIZED LOADS FROM STORED ONCE POINTER: " << *GV);
    ++NumGlobUses;
  }
  ...
  if (AllNonStoreUsesGone) {
    DOUT(llvm::dbgs() << "  *** GLOBAL NOW DEAD!\n");
...
static GlobalVariable *OptimizeGlobalAddressOfMalloc(GlobalVariable *GV,
                                                     MallocInst *MI,
                                                     LLVMContext &Context) {
  DOUT(llvm::dbgs() << "PROMOTING MALLOC GLOBAL: " << *GV << "  MALLOC = " << *MI);
...
static GlobalVariable *PerformHeapAllocSRoA(GlobalVariable *GV, MallocInst *MI,
                                            LLVMContext &Context){
  DOUT(llvm::dbgs() << "SROA HEAP ALLOC: " << *GV << "  MALLOC = " << *MI);
...
static bool TryToShrinkGlobalToBoolean(GlobalVariable *GV, Constant *OtherVal,
  ...
  DOUT(llvm::dbgs() << "   *** SHRINKING TO BOOL: " << *GV);
...
bool GlobalOpt::ProcessInternalGlobal(GlobalVariable *GV,
  ...
  if (!AnalyzeGlobal(GV, GS, PHIUsers)) {
    ...
    if (!GS.HasMultipleAccessingFunctions &&
      ...
      DOUT(llvm::dbgs() << "LOCALIZING GLOBAL: " << *GV);
    ...
    // Delete it now.
    if (!GS.isLoaded) {
      DOUT(llvm::dbgs() << "GLOBAL NEVER LOADED: " << *GV);
      ...
      DOUT(llvm::dbgs() << "MARKING CONSTANT: " << *GV);
      GV->setConstant(true);
      ...
      if (GV->use_empty()) {
        DOUT(llvm::dbgs() << "   *** Marking constant allowed us to simplify "
      ...
      if (Constant *SOVConstant = dyn_cast<Constant>(GS.StoredOnceValue))
        if (isa<UndefValue>(GV->getInitializer())) {
          ...
          if (GV->use_empty()) {
            DOUT(llvm::dbgs() << "   *** Substituting initializer allowed us to "
