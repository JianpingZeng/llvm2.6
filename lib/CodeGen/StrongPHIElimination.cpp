...
void StrongPHIElimination::processBlock(MachineBasicBlock* MBB) {
  ...
  while (P != MBB->end() && P->getOpcode() == TargetInstrInfo::PHI) {
    ...
    for (std::map<unsigned, MachineBasicBlock*>::iterator QI = PHIUnion.begin(),
      ...
      DOUT(llvm::dbgs() << "Adding Renaming: " << QI->first << " -> "
           << P->getOperand(0).getReg() << "\n");
    }
  ...
  while (!worklist.empty() || !copy_set.empty()) {
    while (!worklist.empty()) {
      ...
      if (isLiveOut(curr.second, MBB, LI)) {
        ...
        DOUT(llvm::dbgs() << "Inserted copy from " << curr.second << " to " << t << "\n");
      ...
      DOUT(llvm::dbgs() << "Inserted copy from " << curr.first << " to "
           << curr.second << "\n");
...
bool StrongPHIElimination::runOnMachineFunction(MachineFunction &Fn) {
  ...
  for (RenameSetType::iterator I = RenameSets.begin(), E = RenameSets.end();
    ...
    for (std::map<unsigned, MachineBasicBlock*>::iterator
      ...
      if (!seen.count(OI->first)) {
        ...
        ++OI;
        I->second.erase(reg);
        DOUT(llvm::dbgs() << "Removing Renaming: " << reg << " -> " << I->first << "\n");
      }
    }
  }
  ...
  for (RenameSetType::iterator I = RenameSets.begin(), E = RenameSets.end();
    ...
    while (I->second.size()) {
      ...
      DOUT(llvm::dbgs() << "Renaming: " << SI->first << " -> " << I->first << "\n");
      ...
      if (SI->first != I->first) {
        if (mergeLiveIntervals(I->first, SI->first)) {
          ...
          DOUT(llvm::dbgs() << "Renaming failed: " << SI->first << " -> "
               << I->first << "\n");
        }
      }
