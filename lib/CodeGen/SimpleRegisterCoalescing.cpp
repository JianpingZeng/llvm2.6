...
bool SimpleRegisterCoalescing::AdjustCopiesBackFrom(LiveInterval &IntA,
  ...
  if (TargetRegisterInfo::isPhysicalRegister(IntB.reg) &&
    ...
    for (const unsigned* SR = tri_->getSubRegisters(IntB.reg); *SR; ++SR)
      if (li_->hasInterval(*SR) && IntA.overlaps(li_->getInterval(*SR))) {
        DOUT(llvm::dbgs() << "Interfere with sub-register ");
        DEBUG(li_->getInterval(*SR).print(llvm::dbgs(), tri_));
        return false;
      }
  }
  
  DOUT(llvm::dbgs() << "\nExtending: "); IntB.print(llvm::dbgs(), tri_);
  ...
  DOUT(llvm::dbgs() << "   result = "; IntB.print(llvm::dbgs(), tri_));
  DOUT(llvm::dbgs() << "\n");
...
bool SimpleRegisterCoalescing::RemoveCopyByCommutingDef(LiveInterval &IntA,
  ...
  DOUT(llvm::dbgs() << "\nShortening: "); IntA.print(llvm::dbgs(), tri_);
  IntA.removeValNo(AValNo);
  DOUT(llvm::dbgs() << "   result = "); IntA.print(llvm::dbgs(), tri_);
  DOUT(llvm::dbgs() << "\n");
...
SimpleRegisterCoalescing::CanJoinExtractSubRegToPhysReg(unsigned DstReg,
  ...
  if (li_->hasInterval(RealDstReg) &&
    ...
    DOUT(llvm::dbgs() << "Interfere with register ");
    DEBUG(li_->getInterval(RealDstReg).print(llvm::dbgs(), tri_));
...
SimpleRegisterCoalescing::CanJoinInsertSubRegToPhysReg(unsigned DstReg,
  ...
  if (li_->hasInterval(RealSrcReg) &&
    ...
    DOUT(llvm::dbgs() << "Interfere with register ");
    DEBUG(li_->getInterval(RealSrcReg).print(llvm::dbgs(), tri_));
    return false; // Not coalescable
  }
...
bool SimpleRegisterCoalescing::JoinCopy(CopyRec &TheCopy, bool &Again) {
  ...
  DOUT(llvm::dbgs() << li_->getInstructionIndex(CopyMI) << '\t' << *CopyMI);
  ...
  if (isExtSubReg) {
    ...
    if (SrcSubIdx && SrcSubIdx != DstSubIdx) {
      ...
      DOUT(llvm::dbgs() << "\tSource of insert_subreg is already coalesced "
           << "to another register.\n");
      return false;  // Not coalescable.
    }
  ...
  if (SrcReg == DstReg) {
    DOUT(llvm::dbgs() << "\tCopy already coalesced.\n");
    return false;  // Not coalescable.
  }
  ...
  if (SrcIsPhys && DstIsPhys) {
    DOUT(llvm::dbgs() << "\tCan not coalesce physregs.\n");
    return false;  // Not coalescable.
  }
  
  // We only join virtual registers with allocatable physical registers.
  if (SrcIsPhys && !allocatableRegs_[SrcReg]) {
    DOUT(llvm::dbgs() << "\tSrc reg is unallocatable physreg.\n");
    return false;  // Not coalescable.
  }
  if (DstIsPhys && !allocatableRegs_[DstReg]) {
    DOUT(llvm::dbgs() << "\tDst reg is unallocatable physreg.\n");
    return false;  // Not coalescable.
  }
  ...
  if (isExtSubReg || isInsSubReg || isSubRegToReg) {
    ...
    if (SrcIsPhys && isExtSubReg) {
      ...
      if (DstSubIdx) {
        ...
        // coalesced to a larger register so the subreg indices cancel out.
        if (DstSubIdx != SubIdx) {
          DOUT(llvm::dbgs() << "\t Sub-register indices mismatch.\n");
          return false; // Not coalescable.
        }
      } else
      ...
      if (!isSubRegToReg && CopyMI->getOperand(1).getSubReg()) {
        DOUT(llvm::dbgs() << "\tSrc of extract_subreg already coalesced with reg"
             << " of a super-class.\n");
        return false; // Not coalescable.
      }
      ...
      if (SubIdx) {
        ...
        if (!NewRC) {
          DOUT(llvm::dbgs() << "\t Conflicting sub-register indices.\n");
          return false;  // Not coalescable
        }
    ...
    if (!SrcIsPhys && !DstIsPhys &&
      ...
      DOUT(llvm::dbgs() << "\tSrc/Dest are different register classes.\n");
  ...
  DOUT(llvm::dbgs() << "\t\tInspecting "); SrcInt.print(llvm::dbgs(), tri_);
  DOUT(llvm::dbgs() << " and "); DstInt.print(llvm::dbgs(), tri_);
  DOUT(llvm::dbgs() << ": ");
      ...
      if (PhysJoinTweak) {
        if (SrcIsPhys) {
          if (!isWinToJoinVRWithSrcPhysReg(CopyMI, CopyMBB, DstInt, SrcInt)) {
            ...
            ++numAborts;
            DOUT(llvm::dbgs() << "\tMay tie down a physical register, abort!\n");
            Again = true;  // May be possible to coalesce later.
            return false;
          }
        } else {
  ...
  DOUT(llvm::dbgs() << "\n\t\tJoined.  Result = "); ResDstInt->print(llvm::dbgs(), tri_);
  DOUT(llvm::dbgs() << "\n");
...
void SimpleRegisterCoalescing::joinIntervals() {
  DOUT(llvm::dbgs() << "********** JOINING INTERVALS ***********\n");
...
bool SimpleRegisterCoalescing::runOnMachineFunction(MachineFunction &fn) {
  ...
  if (EnableJoining) {
    joinIntervals();
    DEBUG({
        DOUT(llvm::dbgs() << "********** INTERVALS POST JOINING **********\n");
        for (LiveIntervals::iterator I = li_->begin(), E = li_->end(); I != E; ++I){
          I->second->print(llvm::dbgs(), tri_);
          DOUT(llvm::dbgs() << "\n");
        }
      });
  }
