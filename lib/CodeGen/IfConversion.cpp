  ...
  while (IfCvtLimit == -1 || (int)NumIfCvts < IfCvtLimit) {
    ...
    while (!Tokens.empty()) {
      ...
      case ICSimpleFalse: {
        ...
        DOUT(llvm::dbgs() << "Ifcvt (Simple" << (Kind == ICSimpleFalse ? " false" :"")
             << "): BB#" << BBI.BB->getNumber() << " ("
             << ((Kind == ICSimpleFalse)
                 ? BBI.FalseBB->getNumber()
                 : BBI.TrueBB->getNumber()) << ") ");
        RetVal = IfConvertSimple(BBI, Kind);
        DOUT(llvm::dbgs() << (RetVal ? "succeeded!" : "failed!") << "\n");
        if (RetVal) {
      ...
      case ICTriangleFRev: {
        ...
        DOUT(llvm::dbgs() << "Ifcvt (Triangle");
        if (isFalse)
          DOUT(llvm::dbgs() << " false");
        if (isRev)
          DOUT(llvm::dbgs() << " rev");
        DOUT(llvm::dbgs() << "): BB#" << BBI.BB->getNumber() << " (T:"
      ...
      case ICDiamond: {
        ...
        DOUT(llvm::dbgs() << "Ifcvt (Diamond): BB#" << BBI.BB->getNumber() << " (T:"
             << BBI.TrueBB->getNumber() << ",F:"
             << BBI.FalseBB->getNumber() << ") ");
